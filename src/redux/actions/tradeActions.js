...
export const handleAddTradeAcct =
  ...
  (dispatch) => {
    ...
    axios
      .post(
        `https://brain.evergreenffx.com/v2/?action=add_trade_acct&user_id=${user_id}&account_id=${account_id}&account_name=${account_name}&account_password=${encodeURIComponent(
          account_password
        )}&broker=${brokerNo}&broker_server=${broker_server}&broker_server_id=${broker_server_id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
