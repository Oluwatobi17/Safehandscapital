...
const AccountSettings = () => {
  ...
  if (userInfo.info) {
    ...
    email = userInfo.info[0]?.email;
  } else {
    user_id = userInfo.userInfo?.data[0]?.id;
    email = userInfo.userInfo?.data[0]?.email;
  }
  ...
  const Password = () => {
    ...
    const handleSubmit = () => {
      dispatch(update_password(password, user_id, token, navigate, toast)).then(
        () => {
          dispatch(signout());
        }
      );
