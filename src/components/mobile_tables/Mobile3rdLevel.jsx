...
const Mobile3rdLevel = ({ thirdLevel }) => {
  const navigate = useNavigate();
  const [active, setActive] = useState([]);

  const firstData = [
    {
      fullName: thirdLevel?.data[0]?.fullname,
      userName: thirdLevel?.data[0]?.username,
      referrals: thirdLevel?.data[0]?.no_of_refs,
      status: thirdLevel?.data[0]?.status,
    },
  ];
